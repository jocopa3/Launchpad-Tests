package com.jocopa3.launchpad.effects;

import static com.jocopa3.launchpad.effects.NoiseUtils.*;
...
public class SimplexNoise {
    ...
    //Implement 2D simplex algorithm
    ...
    public float octaveNoise3D(int oct, float x, float y, float z) {
        float value = 0.0f;
        int max = 0;
        int octs = octaves == 0 ? oct : octaves;
    ...
    public float octaveNoise3D(int oct, float fre, float amp, float x, float y, float z) {
        float value = 0.0f;
        float ampl = 1, freq = 1, max = 0;
        int octs = octaves == 0 ? oct : octaves;
        for (int i = 0; i < octs; i++) {
            //value += noise(
            //	(x * (2 << (i - 1))),
    ...
    public double getValue(double x, double y) {
        return octaveNoise2D(-1, (float)x, (float)y);
    }
    
    public float getValue(float x, float y){
        return octaveNoise2D(-1, x, y);
    }
}
