 ...
 *
 * This file is part of a library called The MidiBus (themidibus) -
 * http://www.smallbutdigital.com/themidibus.php.
 *
 * The MidiBus is free software: you can redistribute it and/or modify it under
 * the terms of the GNU General Public License as published by the Free Software
 * Foundation, either version 3 of the License, or (at your option) any later
 * version. * The MidiBus is distributed in the hope that it will be useful, but
 * WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE. See the GNU General Public License for more
 * details.
 *
 * You should have received a copy of the GNU General Public License along with
 * the MidiBus. If not, see <http://www.gnu.org/licenses/>.
 */
package themidibus;
 ...
 * {@link #sendNoteOn(int channel, int pitch, int velocity)}, {@link #sendNoteOff(int channel, int pitch, int velocity)}
 * and {@link #sendControllerChange(int channel, int number, int value)} and
 * receive midi via the PApplet methods this package provides support for such
 * as
 * {@link PApplet#noteOn(int channel, int pitch, int velocity)}, {@link PApplet#noteOff(int channel, int pitch, int velocity)}
 * and {@link PApplet#controllerChange(int channel, int number, int value)}.
...
public class MidiBus {
     ...
     * other) for callbacks. No input or output MIDI devices will be opened. The
     * new MidiBus's bus_name will be generated automatically.
     *
     ...
     * specified parent (PApplet or other) for callbacks. No input or output
     * MIDI devices will be opened.
     *
     * @param parent the parent to which this MidiBus is attached.
    ...
    /**
     * Constructs a new MidiBus and registers the specified parent (PApplet or
     * other) for callbacks. Opens the MIDI input and output devices specified
     * by the indexes in_device_num and out_device_num. A value of -1 can be
     * passed to in_device_num if no input MIDI device is to be opened, or to
     * out_device_num if no output MIDI device is to be opened. The new
    ...
    /**
     * Constructs a new MidiBus with the specified bus_name and registers the
     * specified parent (PApplet or other) for callbacks. Opens the MIDI input
     * and output devices specified by the indexes in_device_num and
     * out_device_num. A value of -1 can be passed to in_device_num if no input
     * MIDI device is to be opened, or to out_device_num if no output MIDI
     ...
     * Constructs a new MidiBus and registers the specified parent (PApplet or
     * other) for callbacks. Opens the MIDI input and output devices specified
     * by the names in_device_name and out_device_name. An empty String can be
     * passed to in_device_name if no input MIDI device is to be opened, or to
     * out_device_name if no output MIDI device is to be opened. The new
     * MidiBus's bus_name will be generated automatically.
     * <p>
     * If two or more MIDI inputs have the same name, whichever appears first
     * when {@link #list()} is called will be added, simlarly for two or more
     * MIDI outputs with the same name. If this behavior is problematic use
     ...
     * Constructs a new MidiBus with the specified bus_name and registers the
     * specified parent (PApplet or other) for callbacks. Opens the MIDI input
     * and output devices specified by the names out_device_name and
     * out_device_name. An empty String can be passed to in_device_name if no
     * input MIDI device is to be opened, or to out_device_name if no output
     * MIDI device is to be opened.
     * <p>
     * If two or more MIDI inputs have the same name, whichever appears first
     * when {@link #list()} is called will be added, simlarly for two or more
     * MIDI outputs with the same name. If this behavior is problematic use
    ...
    /**
     * More flavors of constructor, similar to the others, but with mixed
     * arguments
     *
     * @param parent the parent to which this MidiBus is attached.
    ...
    /**
     * Perfoms the initialisation of new MidiBus objects, is private for a
     * reason, and is only ever called within the constructors. This method
     * exists only for the purpose of cleaner and easier to maintain code.
     * Creates a new (hopefully/probably) unique bus_name value for new MidiBus
     * objects that weren't given one. If two MidiBus object were to have the
     * same name, this would be bad, but not fatal, so there's no point in
     ...
     * be added again.
     *
     * @param device_info the MidiDevice.Info of the MIDI input device to be
     * added.
     * @return true if and only if the input device was successfully added.
     */
     ...
     * not be added again.
     *
     * @param device_info the MidiDevice.Info of the MIDI output device to be
     * added.
     * @return true if and only if the input device was successfully added.
     */
    ...
    /**
     * Notifies any of the supported methods implemented inside the parent of a
     * new MIDI message from one of the MIDI input devices.
     *
     * @param message the new inbound MidiMessage.
     */
    ...
    /* -- Listener Handling -- */
    /**
     * Registers an Object as the parent in order to recieve method callbacks as
     * per {@link PApplet}. Calling this will replace the previous parent Object
     * if any was set.
     *
    ...
    /**
     * Configure this MidiBus instance to send or not to send timestamps along
     * with MIDI information to the MIDI subsystem.
     *
    ...
    /**
     * Rescan for Midi Devices. This is autocalled once when the MidiBus starts
     * up. It should be called again if you need to refresh the list of
     * available MidiDevices while your program is running.
     *
     */
